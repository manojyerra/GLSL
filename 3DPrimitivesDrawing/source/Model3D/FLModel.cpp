...
Shape* FLModel::AddBestBoudingShapeByVerticesOnRect(Rect2D* rect)
{
	return _boundingShapes->AddBestBoudingShapeByVerticesOnRect(rect, (float*)_verticesPointer, _numVertex, _mat.m);
}

Shape* FLModel::AddBoudingShapeByVerticesOnRect(Rect2D* rect, int boundingShapeID)
{
...
void FLModel::Draw()
{
	...
	if(_wireFrameLinesEnabled || _wireFramePointsEnabled)
	{
		...
		{
			GLfloat lineWidth = GLUtil::GLLineWidth(0.50);
...
void FLModel::DrawLocalAxis()
{
	...
	for(int i=0; i<3; i++)
	{
		...
		if(i == 0)		glColorui(0xff0000ff);
		else if(i == 1)	glColorui(0x00ff00ff);
		else if(i == 2)	glColorui(0x0000ffff);
...
Rect2D FLModel::GetBounding2DRect(bool multWithLocalMat)
{
...
{
	Rect2D rect = GetBounding2DRect(false);
...
{
	Rect2D rect = GetBounding2DRect(true);
